...
class BillingController {
     ...
     */
    async downloadInvoicePdfAsAttachment(req, res) {
        try {
            const invoiceId = req.params.id;
            const invoice = await invoice_model_1.Invoice.findById(invoiceId);
            if (!invoice) {
            ...
            try {
                if (provider.contactEmail) {
                    await this.emailService.sendInvoiceEmail(provider.contactEmail, `${invoice.puntoVenta}-${invoice.numero}`, pdfBuffer);
